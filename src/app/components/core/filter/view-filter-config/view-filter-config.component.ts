...
@Component({
  selector: 'app-view-filter-config',
  templateUrl: './view-filter-config.component.html',
  styleUrls: ['./view-filter-config.component.css']
})
export class ViewFilterConfigComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    this.filesToUpload = [];
    this.configItems = [];
    if (changes.filterConfig.currentValue !== undefined) {
      let configAttributes = [];
      if (changes.filterConfig.currentValue.length !== 0) {
        const currentConfigValues = changes.filterConfig.currentValue;
        console.log('Filter config', currentConfigValues);
        ...
        changes.filterConfig.currentValue.value = configAttributes;
        this.categoryConfiguration = changes.filterConfig.currentValue;
